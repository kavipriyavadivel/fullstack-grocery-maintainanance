...
const Admindayreq = () => {
  ...
  const [error, setError] = useState(null);
      ...
      const fetchData = async () => {
        try {
          const response = await fetch(
             "http://localhost:8080/grocery/day-pending-list"
          );
          if (!response.ok) {
    ...
    const handleStatus = async (id) =>{
      ...
      try{
         await fetch( `http://localhost:8080/grocery/day-status-change/${id}`,{
          method:"PUT",
          headers:{
    ...
    <div className='issue-item'>
      ...
      <div className="issue-table">
        <div style={{ textAlign: "center" }}>
          <h1>Day Scholar Request List</h1>
        </div>
